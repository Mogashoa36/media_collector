...
export function renderGallery(gallery, videos, handlers) {
  ...
  gallery.innerHTML = videos.map(video => { const thumbnail = typeof video.thumbnail === 'object' ? video.thumbnail : { url: video.thumbnail }; const duration = video.video?.duration; const resolution = video.video?.width && video.video?.height ? `${video.video.width}×${video.video.height}` : ''; const downloadable = handlers.canDownload?.(video) ?? false; return `<article class="video-card" data-id="${video.id}"><div class="thumbnail-wrap"><img src="${escapeHtml(thumbnail.url || placeholderThumbnail())}" alt="" loading="lazy">${duration ? `<span class="duration-badge">${formatDuration(duration)}</span>` : ''}<label class="card-select"><input type="checkbox" data-action="select" aria-label="Select ${escapeHtml(video.title)}"><span></span></label><div class="play-overlay">▶</div><button class="card-favorite ${video.favorite ? 'is-favorite' : ''}" data-action="favorite" title="${video.favorite ? 'Unfavorite' : 'Favorite'}">${video.favorite ? '♥' : '♡'}</button></div><div class="card-content"><div class="card-title">${escapeHtml(video.title)}</div><div class="card-meta"><span class="platform-dot"></span>${escapeHtml(video.platform)} <span>•</span> ${formatDate(video.savedAt)}${resolution ? ` <span>•</span> ${resolution}` : ''}${downloadable ? '<button class="card-download" data-action="download" title="Download">⬇</button>' : ''}<button class="more-button" data-action="details" aria-label="More options">•••</button></div></div></article>`; }).join('');
  gallery.querySelectorAll('.video-card').forEach(card => card.addEventListener('click', event => {
    ...
    if (action === 'select') { handlers.select(video, event.target.checked, card); event.stopPropagation(); } else if (action === 'favorite') handlers.favorite(video); else if (action === 'download') { handlers.download(video); event.stopPropagation(); } else if (action === 'details') handlers.details(video); else handlers.open(video);
  }));
}